...
# This is the most time-consuming leetcode problem I've seen so far, but the concepts touched upon have made this problem worthwhile.
...
class Solution:
    ...
    def backtrack(self, start, end, dictionary):
        connection_nodes, distance_left, distance_right = self.get_connection_nodes(start, end, dictionary)
        if distance_left == distance_right == 1:
            return [[start, node, end] for node in connection_nodes]
        elif distance_left == 1 and distance_right == 0:
            return [[start, end]]
        paths = []
        for node in connection_nodes:
            left_paths = self.backtrack(start, node, dictionary)
            right_paths = self.backtrack(node, end, dictionary)
            paths.extend( [l[:-1] + r for l in left_paths for r in right_paths] )
        return paths
    ...
    def findLadders(self, start, end, dictionary): # dict is reserved and should not be used as parameter name
        return self.backtrack(start, end, dictionary)
